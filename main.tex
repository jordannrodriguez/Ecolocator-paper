...
For individuals from locally adapted populations, 
predicting the climate to which their genotypes are best matched provides a 
reference for evaluating suitability under present and future conditions.
...
with genotype--environment association (GEA) methods identifying environmentally 
associated loci and 
machine-learning models demonstrating that geographic origin can be predicted 
directly from genotypes. 
Here we introduce \ecolocator, 
a supervised neural network that jointly predicts geographic location 
and climate of origin from genotypes. 
In spatially explicit simulations, 
increasing dispersal reduced both geographic and environmental prediction accuracy, 
whereas weakening selection reduced environmental prediction while leaving 
geographic prediction largely unchanged, consistent with distinct spatial and adaptive signals. 
SHAP-based feature attribution preferentially ranked adaptive loci and outperformed 
benchmark GEA approaches across the simulation regimes tested.
To test our model in a natural system, 
we applied \ecolocator to coastal \dfir (\textit{Pseudotsuga menziesii var. menziesii}). 
Under leave-one-out cross-validation, 
\ecolocator predicted 
latitude with $R^2=0.83$, 
longitude with $R^2=0.75$, 
and three climate variables with $R^2=0.52$--$0.72$. 
Importantly, climate predictions made directly from genotypes were more accurate 
than climates obtained by first predicting geographic origin and then looking up 
the historical climate at that location, showing that \ecolocator recovers genotype--climate 
signals that cannot be reconstructed from geographic prediction alone. 
Most predictions also fell within established seed-zone climate tolerances, 
placing prediction errors within ranges already used to guide seed-transfer decisions. 
Available through a Python API and command-line interface, 
\ecolocator is open-source software that recovers spatial and 
climate-associated genomic signals and translates them into interpretable 
information for studying and managing locally adapted populations 
under rapid environmental change. %JR- Im unsure if i really hate the whole abstract, maybe I've tried too hard for too long on it. Need fresh eyes. Also, Im sure it needs to be way shortened. rip
...
deme and $s$ is a fixed selection coefficient, with 
...
assembled ($\sim$16 Gb; \cite{neale_douglas-fir_2017}),
...
from SHAP rankings --- comparable to a $p$-value for ranking variants by
evidence of adaptation, but calibrated against known true adaptive status
...
transcript-level origin, so ``loci'' here refers to SNPs anchored within or
near an expressed gene rather than an anonymous genomic position matched to
the nearest annotated gene. Among loci with informative annotations,
...
northern Cascades are topographically steeper. This pattern instead
...
More broadly,
the approach is not limited to \dfir
...
can be analysed with \ecolocator,
making it a useful tool for less-studied species
...
These considerations do not preclude application,
but they set the conditions under which predictions can be trusted,
and they make interpreting predictions within the biological
and management context of each application essential.
...
Rather than regressing geographic structure out prior to environmental prediction, 
\ecolocator models geographic and environmental variation as related but distinct
\ecolocator takes a different route: rather than regressing geographic structure out, it models geographic and environmental variation as related but distinct prediction tasks. 
However, this does not resolve the confounding. When climate
covaries with geography, non-causal but spatially structured loci can still
 receive high attribution for climate simply because they predicts location, and
 \ecolocator does not attempt to ask if given locus carries environmental signal beyond
  what spatial structure explains, as in more classic GEA methods. 
  Attribution from \ecolocator is best interpretted as
  identifying loci that are informative for the joint prediction task, with the
  question of which of those are adaptive left to the follow-up analyses
  discussed above.
...
or subsequent experimental studies.
