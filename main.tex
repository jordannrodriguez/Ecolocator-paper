...
in \dfir is limited (typically $<400$~km),
...
Under restricted dispersal (d = 0.1) and
...
(d = 0.1, Lon. $R^2$=0.968, Lat. $R^2$=0.967, Env. $R^2$=0.769;
\Cref{fig:sim-performance}, panel a),
while increasing dispersal reduced accuracy
for both geographic and environmental prediction 
(d=0.5, Lon. $R^2$=0.815, Lat. $R^2$=0.814, Env. $R^2$=0.336; 
\Cref{fig:sim-performance}, panel a).
...
and on an H=0 landscape geography carries no information about the environmental target.
...
($\vec{\Delta}$LA=0.008 between GA1 and GA3),
...
\Cref{fig:permutation}). Random genome-wide permutation produced a more 
...
(\Cref{fig:gea-comparison}). The random baseline represents the AUC-PR
...
\begin{figure}[H]
    ...
    (SHM; $R^2$= 0.52), and Mean Summer Precipitation (MSP; $R^2$= 0.54).
    \textbf{Panels f-g.}
...
for all three variables (\Cref{tab:supptable1}),
...
Precipitation (MSP) ($R^2$=0.54) and Summer Heat:Moisture Index (SHM) ($R^2$=0.52)
...
than the longitudinal winter-temperature gradient captured by MCMT.

Functional annotation of the high-attribution SNPs revealed additional 
differences among prediction targets. Among loci with informative annotations, 
...
environment from geography, but environmental gradients commonly
covary with geographic structure in natural landscapes
...
Discrete demes, migration only between neighbors,
and directional selection on a dozen causal loci
give the model a clearly structured task.
Continuous space, diffuse genetic architectures,
high dispersal, or weak selection all erode the signals
that \ecolocator is designed to learn,
and the contrast between the two regimes is a reminder that
the difficulty of the prediction problem is depends on the biology
...
Our continuous-space simulations hold climate independent of geography ($H=0$).
...
Thus, \ecolocator is not simply handing the same spatially structured loci
high attribution for every target; a good part of the signal
is specific to each axis of geographic or environmental variation.
The functional annotations point the same way and line up with
what is already known in coastal \dfir.
Latitude candidates were enriched for light sensing and seasonal
developmental regulation, which fits the known effects of photoperiod
and temperature on growth cessation \citep{ford_photoperiod_2017}.
Longitude and MCMT candidates leaned toward seasonal regulation
and temperature acclimation, in line with genomic evidence for
polygenic cold adaptation in this species \citep{delatorre_polygenic_2021}.
...
Finally,
\dfir populations east of the Cascade crest show evidence of introgression
with interior \dfir
...
when evaluated with a model trained primarily on coastal \dfir
...
associated with environmental variation. This design also addresses a well-documented 
...
\begin{figure}[H]
    ...
    3; $d$ = 0.1 and \(\sigma_K\) = 0.5 fixed): distributing QTLs increasingly across the
    ...
    Selection (\(\sigma_K\) = 0.5, 0.7, 0.9; genetic architecture 1 and $d$ = 0.3 fixed): 
...
\begin{figure}[H]
    ...
    simulated data ($d$ = 0.1, \(\sigma_K\) = 0.5, $g$ = 1). In the top row, there are 1000 SNPs
...
\begin{figure}[H]
    ...
    strong-selection regime (d=0.1, \(\sigma_K\)=0.5). Right: higher-dispersal, 
    weaker-selection regime (d=0.5, \(\sigma_K\)=0.7). Prediction accuracy 
...
\begin{table}[ht]
  \centering
  \caption{\textbf{$R^2$ values for \ecolocator climate variable predictions versus ClimateNA modeled values from predicted locations.} Values were calculated 
  separately for this direct comparison between \ecolocator climate predictions and 
  ClimateNA estimates and therefore differ slightly from the primary leave-one-out cross-validation 
  metrics reported in the main text.}
  ...
  Mean Summer Precipitation (MSP, mm) & 0.51 & 0.31 \\
  Summer Heat:Moisture Index (SHM) & 0.53 & 0.30 \\
  \hline
  \end{tabular}
\end{table}
