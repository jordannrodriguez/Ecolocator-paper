...
and climate of origin from genotypes. 
In both spatially explicit (non-Wright--Fisher) and deme-based (Wright--Fisher) simulations,
\ecolocator accurately recovered geographic location and environment of origin from genotypes,
and SHAP-based feature attribution identified adaptive loci more reliably than benchmark GEA methods.
Applied to coastal \dfir (\textit{Pseudotsuga menziesii var. menziesii}),
\ecolocator predicted geographic origin ($R^2=0.75$--$0.83$) and climate of origin
($R^2=0.52$--$0.72$) under leave-one-out cross-validation.
Notably, climate predicted directly from genotypes outperformed climate inferred by first
...
These prediction errors fell within the tolerances used in existing seed-transfer guidelines,
demonstrating that \ecolocator's predictions are precise enough for practical use, an approach
...
information for studying and managing populations
